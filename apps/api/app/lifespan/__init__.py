...
# Importing the task modules is what registers their handlers. Without this the
# API can construct a queue but every enqueue() raises UnknownTaskError.
...
from fastforge_jobs import JobsSettings, create_task_queue
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None]:
    ...
    app.state.task_queue = create_task_queue(jobs_settings)
    logger.info(
        "API startup complete",
        mail_provider=mail_settings.provider,
        cache_provider=cache_settings.provider,
        jobs_provider=jobs_settings.provider,
    )
    try:
        yield
    finally:
        # Drain before the DB closes: an in-memory job may still be mid-flight
        # and holding a session.
