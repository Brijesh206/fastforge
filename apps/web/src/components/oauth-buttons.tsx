...
/** Brand marks, inlined so no icon dependency is needed — lucide (the icon set
 *  already in use) deliberately ships no third-party logos. */
const GoogleIcon = () => (
  ...
  return (
    <div className="space-y-2">
      {PROVIDERS.map(({ name, label, Icon }) => (
        <Button key={name} asChild variant="outline" className="w-full">
          <a href={`${API_URL}/auth/${name}/login`}>
            <Icon />
            Continue with {label}
          </a>
        </Button>
